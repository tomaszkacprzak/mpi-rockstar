...
void reset_error() {
    ...
    for (i = 0; i < num_clients; i++) {
        if (recv_from_socket(clients[i].cs, cmd, 4) <= 0) {
            fprintf(stderr, "[Error] Fatal error: could not recover from "
                            "network failure!\n");
            shutdown_clients();
            exit(1);
        }
...
void init_clients() {
    for (int64_t i = 0; i < num_clients; i++)
        send_msg(clients[i].cs, clients[i].address,
                 strlen(clients[i].address) + 1);

    for (int64_t i = 0; i < num_clients; i++)
        clients[i].serv_port =
            recv_msg_nolength(clients[i].cs, clients[i].serv_port);
...
void sort_clients() {
    ...
    qsort(clients + NUM_READERS, NUM_WRITERS, sizeof(struct client_info),
          sort_by_address);
}
...
void read_blocks(int64_t snap) {
    for (int64_t reader = 0; reader < NUM_READERS; reader++) {
        ...
        for (block = block_start; block < block_end; block++) {
            ...
            send_to_socket_noconfirm(clients[reader].cs, &snap,
                                     sizeof(int64_t));
            send_to_socket_noconfirm(clients[reader].cs, "rdbk", 4);
            send_to_socket_noconfirm(clients[reader].cs, &block,
                                     sizeof(int64_t));
        }
    }
...
void decide_chunks() {
    ...
    if (strlen(LOAD_BALANCE_SCRIPT))
        decide_chunks_by_script();
    else if (NUM_WRITERS == 1)
        decide_chunks_for_volume_balance();
    else
        decide_chunks_for_memory_balance();
}
...
void decide_boundaries() {
    ...
    for (reader = 0; reader < NUM_READERS; reader++) {
        ...
        if (reader == 0)
            memcpy(all_particle_bounds, clients[reader].bounds,
                   sizeof(float) * 6);
        else
            bounds_union(all_particle_bounds, clients[reader].bounds,
                         all_particle_bounds);
    }
    if ((BOX_SIZE < OVERLAP_LENGTH * 5) && PERIODIC) {
        shutdown_clients();
        fprintf(stderr,
                "[Error] Box size too small (%f) relative to overlap length "
                "(%f)!\n",
...
void check_num_writers(void) {
    ...
    if ((factors[0] < 2) || (factors[1] < 2) || (factors[2] < 2)) {
        fprintf(stderr, "[Error] NUM_WRITERS should be the product of at least "
                        "three factors larger than 1 for periodic boundary "
                        "conditions to be enabled!\n");
        fprintf(stderr,
                "[Error] (Currently, NUM_WRITERS = %" PRId64 " = %" PRId64
                " x %" PRId64 " x %" PRId64 ")\n",
                NUM_WRITERS, factors[0], factors[1], factors[2]);
        fprintf(stderr, "[Error] Please adjust NUM_WRITERS or set PERIODIC=0 "
                        "in the config file.\n");
        exit(1);
    }
}
...
void send_bounds(int64_t i, int64_t j, float *bounds) {
    ...
    send_to_socket_noconfirm(clients[i].cs, clients[j].address,
                             strlen(clients[j].address) + 1);
    send_to_socket_noconfirm(clients[i].cs, clients[j].serv_port,
                             strlen(clients[j].serv_port) + 1);
}
...
void transfer_data(int type, double overlap) {
    ...
    if (type == DATA_BPARTICLES)
        send_cmd = "rcpb";
    if (type == DATA_HPARTICLES)
        send_cmd = "rchp";
    for (; i < max_i; i++) {
        for_writers(j) {
...
void find_halos(int64_t snap) {
    ...
    if (!DUMP_PARTICLES[0]) {
        ...
        if (check_bgc2_snap(snap) || STRICT_SO_MASSES) {
            ...
            for_writers(i) recv_from_socket(
                clients[i].cs, clients[i].halo_bounds, sizeof(float) * 6);
...
void _do_merger_tree_part2(int64_t snap) {
    ...
    for_writers(i) {
        recv_from_socket(clients[i].cs, &(clients[i].head_length),
                         sizeof(int64_t));
        recv_from_socket(clients[i].cs, &(clients[i].cat_length),
                         sizeof(int64_t));
...
void do_merger_tree(int64_t snap) {
    ...
    if (!starting_snap) { // Load in the current snapshot, with overlap
        timestep = 2;
        get_bounds(snap - 1);
        for_writers(i) memcpy(clients[i].bounds_prevsnap, clients[i].bounds,
                              sizeof(float) * 6);

        get_bounds(snap);
        if (!p_bounds)
            check_realloc_s(p_bounds, sizeof(struct prev_bounds), NUM_WRITERS);
        for_writers(i) memcpy(p_bounds[i - NUM_READERS].bounds,
                              clients[i].bounds, sizeof(float) * 6);
        for_writers(i) {
...
int server(void) {
    ...
    int64_t data_size =
        sizeof(struct client_info) * (NUM_READERS + NUM_WRITERS);
