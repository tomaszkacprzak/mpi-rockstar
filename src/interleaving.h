...
*6) From known boundary particles in current chunk, check which boundary
particles in other chunks match up. *7) Find unique gids in other chunks to
request, associated with each boundary fof *8) Sort fofs by size.

Client analysis process:
1) Request fof.
2) If boundary fof, request gid lists in other chunks corresponding to the
collected gids, lock requested gids. If another client with lower chunk number
has asked for a lock, then refuse.  Otherwise, allow. 3) Repeat with the new
gids. 4) Ask for lock on particles from each chunk. 5) Transfer locked particles
6) Run analysis
