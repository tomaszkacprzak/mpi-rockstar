...
void jacobi_decompose(double cov_matrix[][NUM_PARAMS], double *eigenvalues,
                      double orth_matrix[][NUM_PARAMS]) {
    int    i, j, k, l;
...
#define UPDATE(x, y)                                                           \
    ...
    if (changed[x] &&                                                          \
        (fabs(y) <                                                             \
         1e-6 * fabs(eigenvalues[x]))) { /*Eigenvalue didn't change*/          \
        changed[x] = 0;                                                        \
        state--;                                                               \
    } else if (!changed[x] &&                                                  \
               (fabs(y) > 1e-6 * fabs(eigenvalues[x]))) { /*Egval did change*/ \
        changed[x] = 1;                                                        \
        state++;                                                               \
    }
...
#define ROTATE(m, w, x, y, z, r) /*Perform a Jacobi rotation*/                 \
    ...
    if (r) {                                                                   \
        ...
        for (i = 0; i < k; i++) {
            ROTATE(cov_matrix, i, k, i, l, 1);
        }
        for (i = k + 1; i < l; i++) {
            ROTATE(cov_matrix, k, i, i, l, 1);
        }
        for (i = l + 1; i < n; i++) {
            ROTATE(cov_matrix, k, i, l, i, 1);
        }
        for (i = 0; i < n; i++) {
            ROTATE(orth_matrix, k, i, l, i, 0);
        }
...
void set_eig(double input[][3], double *res) {
    ...
    *res = eigs[0];
    ...
    if (eigs[2] < *res)
        *res = eigs[2];
    if (*res <= 0)
        *res = sqrt(input[0][0] + input[1][1] + input[2][2]);
    else {
        *res = sqrt(*res);
    }
}
