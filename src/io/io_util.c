...
size_t fread_swap8(void *ptr, size_t size, size_t nitems, FILE *stream) {
    ...
    n         = check_fread(ptr, size, nitems, stream);
    assert(!((n * size) % 8));
    for (i = 0; i < (n * size); i += 8)
        swap_endian_8byte(e + i);
    return n;
}

size_t _fread_fortran(void *ptr, size_t size, size_t nitems, FILE *stream,
                      int swap) {
    uint32_t head, tail;
    size_t   n;
    if (swap)
        fread_swap(&head, 4, 1, stream);
    else
        check_fread(&head, 4, 1, stream);
    assert((size * nitems) >= head);
    if (swap)
        n = fread_swap(ptr, size, nitems, stream);
    else
        n = check_fread(ptr, size, nitems, stream);
    if (swap)
        fread_swap(&tail, 4, 1, stream);
    else
        check_fread(&tail, 4, 1, stream);
    if (head != tail) {
...
size_t fread_fortran(void *ptr, size_t size, size_t nitems, FILE *stream,
    ...
    int64_t nread = 0;
    while (nread < nitems)
        nread += _fread_fortran(((char *)ptr) + (size * nread), size,
                                nitems - nread, stream, swap);
    return ((size_t)nread);
}
...
size_t fwrite_fortran(void *ptr, size_t size, size_t nitems, FILE *stream) {
    ...
    while (nwritten < nitems) {
        ...
        if (n_to_write > (nitems - nwritten))
            n_to_write = nitems - nwritten;
        _fwrite_fortran(((char *)ptr) + (size * nwritten), size, n_to_write,
                        stream);
        nwritten += n_to_write;
    }
