...
typedef struct bgc2_header {
    ...
    int64_t format_group_data; /* output group data format identifier, see enum
                                  gdata_format below */
    int64_t format_part_data;  /* output particle data format identifier, see
                                  enum pdata_format below */

    int64_t group_type;    /* FOF, SO, etc */
    int64_t ngroups;       /* number of groups stored LOCALLY in this file */
    int64_t ngroups_total; /* number of groups stored GLOBALLY over all output
                              BGC files */

    int64_t
        npart; /* number of particles bound in groups LOCALLY in this file */
    int64_t npart_total; /* number of particles bound in groups GLOBALLY over
                            all output BGC files */
    int64_t npart_orig; /* number of particles from original simulation input */

    int64_t max_npart; /* maximum number of particles in one group LOCALLY in
                          this file */
    int64_t max_npart_total; /* maximum number of particles in one group
                                GLOBALLY over all output BGC files */

    int64_t min_group_part; /* minimum number of particles in a group */
    int64_t
        valid_part_ids; /* valid particle IDs mean they match input snapshot */
    ...
    double bounds[6];  /* Spatial bounds of the halo centers contained in this
                          file */
...
#ifdef BGC2_SIZE
inline size_t bgc_sizeof_gdata(const int64_t gdata_format) {
    switch (gdata_format) {
    ...
    fprintf(stderr,
            "ERROR: unknown group data format!  (format = %" PRId64 ")\n",
            gdata_format);
    return 0;
}
#endif /*BGC2_SIZE*/
...
#ifdef BGC2_SIZE
inline int bgc_format_includes_be(int64_t format_id) {
...
#ifdef BGC2_SIZE
inline size_t bgc_sizeof_pdata(const int64_t pdata_format) {
    switch (pdata_format) {
    ...
    fprintf(stderr,
            "ERROR: unknown particle data format!  (format = %" PRId64 ")\n",
            pdata_format);
    return 0;
}
#endif /*BGC2_SIZE*/
