...
void load_particles_tipsy(char *filename, struct particle **p, int64_t *num_p) {
...
#else
    check_realloc_s((*p),
                    ((*num_p) + header.ndark + header.nsph + header.nstar),
                    sizeof(struct particle));
#endif

    haveiords = load_ids_tipsy(filename, header, &iords, &num_iords);
    if (haveiords &&
        (!iords || num_iords < header.nsph + header.ndark + header.nstar)) {
        fprintf(stderr,
                "[Error] Not enough IDs for total number of particles in Tipsy "
                "file %s!\n",
                filename);
        exit(1);
    }
    ...
    for (i = 0; i < header.nsph; i++) {
        if (xdrfmt)
            ...
            fread((char *)&gas, sizeof(struct tipsy_gas_particle), 1, input);
        for (j = 0; j < 3; j++) {
            if (haveiords)
                (*p)[i + (*num_p)].id = iords[i];
            else
                (*p)[i + (*num_p)].id = i;
            (*p)[i + (*num_p)].pos[j] =
    ...
    fprintf(stderr,
            "[Warning] Rockstar is skipping TIPSY gas and star particles and "
            "calculating halos from dark matter only.  If this is not what you "
            "want, inquire with the authors about Rockstar-Galaxies.\n");
#endif

    for (i = 0; i < header.ndark; i++) {
        int ip = i + header.nsph;
        if (xdrfmt)
            assert(tipsy_xdr_dark(&xdrs, &dark) > 0);
        else
            check_fread((char *)&dark, sizeof(struct tipsy_dark_particle), 1,
                        input);
        if (haveiords)
            (*p)[i + (*num_p)].id = iords[ip];
        else
            (*p)[i + (*num_p)].id = ip;
        for (j = 0; j < 3; j++) {
            (*p)[i + (*num_p)].pos[j] =
    ...
    for (i = 0; i < header.nstar; i++) {
        ...
        if (xdrfmt)
            ...
            fread((char *)&star, sizeof(struct tipsy_star_particle), 1, input);
        for (j = 0; j < 3; j++) {
            if (haveiords)
                (*p)[i + (*num_p)].id = iords[ip];
            else
...
int load_ids_tipsy(char *filename, struct tipsy_dump header, int **iords,
    ...
    if (iordf != NULL) {
        ...
        if ((count == EOF) || (count == 0)) {
            ...
            if ((count == EOF) || (count == 0)) {
                ...
                if (nbodies <= 0 || nbodies > 10000000) {
                    printf("<%s doesn't appear standard or binary or nbodies > "
                           "10 mil.>\n",
                           iofilename);
                    xdr_destroy(&xdrs);
                    fclose(iordf);
        ...
        for (i = 0, count = 0; i < nbodies; i++) {
            int idummy, check = 0;
            if (bStandard)
                check = xdr_int(&xdrs, &idummy);
            else if (bASCII)
                check = (fscanf(iordf, "%d", &idummy) < 1) ? EOF : 0;
            else
                check = check_fread(&idummy, sizeof(int), 1, iordf);
            (*iords)[count] = idummy;
    ...
    if (iordf && (count == nbodies)) {
        ...
        *num_iords = nbodies;
        ...
        fprintf(stderr, "[Warning] Did not read TIPSY iords file; particle IDs "
                        "may not be correct.\n");
        if (*iords)
            free(*iords);
        *iords     = NULL;
...
int tipsy_xdr_gas(XDR *pxdrs, struct tipsy_gas_particle *ph) {
    int i;
    if (!xdr_float(pxdrs, &ph->mass))
        return 0;
    for (i = 0; i < TIPSY_MAXDIM; i++)
        if (!xdr_float(pxdrs, &ph->pos[i]))
            return 0;
    for (i = 0; i < TIPSY_MAXDIM; i++)
        if (!xdr_float(pxdrs, &ph->vel[i]))
            return 0;
    if (!xdr_float(pxdrs, &ph->rho))
        return 0;
    if (!xdr_float(pxdrs, &ph->temp))
        return 0;
    if (!xdr_float(pxdrs, &ph->hsmooth))
        return 0;
    if (!xdr_float(pxdrs, &ph->metals))
        return 0;
    if (!xdr_float(pxdrs, &ph->phi))
        return 0;
    return 1;
}
