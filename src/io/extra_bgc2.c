...
void calc_bgc2_parents(int64_t snap) {
    ...
    for (i = 0; i < NUM_WRITERS; i++) {
        ...
        if (!i)
            hdrs[0].npart_total = hdrs[0].ngroups_total = 0;
    ...
    for (i = 0; i < NUM_WRITERS; i++) {
        if (hdrs[i].ngroups) {
            ...
            key.id      = first_ids[i];
            first_group = bsearch(&key, gd, num_groups,
                                  sizeof(GROUP_DATA_RMPVMAX), sort_by_id);
            assert(first_group);
        } else {
            first_group = gd;
        }
        ...
        fwrite_fortran(first_group, sizeof(GROUP_DATA_RMPVMAX), hdrs[i].ngroups,
                       output);
        fclose(output);
    }
