...
void art_detect_endianness(FILE *input, char *filename) {
    ...
    if (first_word != ART_TEXT_SIZE) {
        fprintf(stderr, "[Error] Unrecognized ART file type in %s!\n",
                filename);
        fprintf(stderr,
                "Expected title size of %d; got %d (or %d if byte-swapped).\n",
...
void art_detect_variant(FILE *input, char *filename) {
    ...
    fprintf(stderr,
            "[Error] Expected first header to be %d or %d bytes; got %d.\n",
            (int32_t)sizeof(struct art_header1),
            (int32_t)sizeof(struct art_header1a), first_word);
    exit(1);
}

void art_process_particles(struct particle *p, int64_t num_p,
    ...
    while ((total_read < np) && !feof(input)) {
        ...
        while ((p_in_record > 0) && !feof(input)) {
            ...
            if (ROCKSTAR_SWAP_ENDIANNESS)
                ...
                num_read = check_fread(pbuffer, sizeof(struct art_particle),
                                       num_read, input);
            art_process_particles(*p, *num_p, pbuffer, num_read,
                                  part_art_header.NGRIDC);
