...
float estimate_scale_radius(float mvir, float rvir, float vmax, float rvmax,
                            float scale) {
    float f, c, vm2;
    if (!mvir || !rvir || !vmax)
        return (rvmax / RMAX_TO_RS);
    vm2 = vmax / VMAX_CONST;
    vm2 = vm2 * vm2 * scale;
...
float calc_scale_from_bins(float rs, float *bin_r, float *weights, 
    ...
    while (fabs(chi2 - last_chi2) > 0.005 * chi2 && iter < MIN_PART_PER_BIN) {
        last_chi2 = chi2;
        if (rs + drs > bin_r[num_bins])
            drs = 0.1 * (bin_r[num_bins] - rs);
        if (rs - drs < bin_r[0])
            drs = 0.1 * (rs - bin_r[0]);
        ...
        if (new_rs > bin_r[num_bins])
            new_rs = rs + 0.8 * (bin_r[num_bins] - rs);
        else if (new_rs < bin_r[0])
            new_rs = rs + 0.8 * (bin_r[0] - rs);
        float new_chi2 = chi2_scale(new_rs, bin_r, weights, num_bins);
        if (chi2_right < new_chi2) {
            new_chi2 = chi2_right;
            new_rs   = rs + drs;
        }
        if (chi2_left < new_chi2) {
            new_chi2 = chi2_left;
            new_rs   = rs - drs;
        }
