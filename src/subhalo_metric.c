...
#pragma omp threadprivate(subtree, sub_metric, subtree_res, alloced_metrics)
...
void build_subtree(struct halo **subs, int64_t num_subs) {
    ...
    if (num_subs > alloced_metrics) {
        ...
        sub_metric =
            check_realloc(sub_metric, sizeof(struct halo_metric) * num_subs,
    ...
    if (subtree)
        fast3tree_rebuild(subtree, num_subs, sub_metric);
    else {
...
float calc_particle_dist(struct halo *h, struct particle *part) {
    ...
    if (h->vrms <= 0 || h->r <= 0)
        return 1e20;
    for (i = 0; i < 3; i++) {
        dx = h->pos[i] - part->pos[i];
        r2 += dx * dx;
    }
    for (; i < 6; i++) {
        dx = h->bulkvel[i - 3] - part->pos[i];
        v2 += dx * dx;
    }
...
int64_t node_could_be_better(struct tree3_node *n, struct particle *part,
    ...
    int64_t i;
    for (i = 0; i < 3; i++)
        if ((part->pos[i] + max_r < n->min[i]) ||
            (part->pos[i] - max_r > n->max[i]))
            return 0;
    return 1;
}
...
double calc_expected_density(struct halo *h, struct particle *part) {
    ...
    r    = sqrt(r2);
    prof = 1.0 + 10.0 * r / h->r;
    return (exp(-0.5 * (v2 / (h->vrms * h->vrms))) /
            (h->vrms * h->vrms * h->vrms) * h->m /
            (h->r * h->r * r * (prof * prof)));
}
...
struct halo_metric **find_children(struct halo *h, struct halo *parent, float r,
    ...
    for (i = 0; i < subtree_res->num_points; i++) {
        swap = 0;
        if (subtree_res->points[i]->target->r >= h->r)
            swap = 1;
        for (ds = 0, j = 0; j < 3; j++) {
        ...
        if (parent && (calc_halo_dist(h, subtree_res->points[i]->target) >
            ...
            swap = 1;
        if (swap) {
            subtree_res->points[i] =
                subtree_res->points[subtree_res->num_points - 1];
