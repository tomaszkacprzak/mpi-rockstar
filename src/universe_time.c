...
/*int main(void) {
  ...
  for (i=0; i<100; i++) {
    ...
    printf("a = %10f t = %10f exact func = %10f exact2 func = %10f interp func =
%10f (frac error in time = %g)\n", a, i/fact-exact_t0_conv,
exact_scale_to_time(a), exact_scale_to_time(a), scale_to_time(a),
scale_to_time(a)/(i/fact-exact_t0_conv)-1.0);
  }

  printf("%f\n", fact);

  fprintf(stderr,"age of universe = %f\n",exact_t0_conv);
  fprintf(stderr,"scale(age of universe) =
