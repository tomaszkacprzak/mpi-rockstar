...
static inline void add_to_previous_halos(struct halo                 *h,
                                         struct binary_output_header *bh,
                                         void                        *file) {
    if (!(num_prev_halos % 1000))
        check_realloc_s(ph, sizeof(struct previous_halo),
                        (num_prev_halos + 1000));
    ...
    ph[num_prev_halos].particles =
        (void *)(file + sizeof(struct binary_output_header) +
                 sizeof(struct halo) * bh->num_halos +
                 h->p_start * sizeof(int64_t));
    num_prev_halos++;
}

void *add_new_prev_file(char *filename, int64_t chunk) {
    for (int64_t i = 0; i < num_prev_files; i++)
        if (prev_chunks[i] == chunk)
            return NULL;
    if (!(num_prev_files % 10)) {
        check_realloc_s(prev_files, sizeof(void *), (num_prev_files + 10));
        check_realloc_s(prev_file_lengths, sizeof(int64_t),
                        (num_prev_files + 10));
...
void load_prev_binary_halos(int64_t snap, int64_t chunk, float *bounds,
                            int64_t our_chunk) {
    void                       *input;
    char                        buffer[1024];
    struct binary_output_header bh;
    ...
    while (remaining > 0) {
        ...
        for (i = 0; i < to_read; i++) {
            for (j = 0; j < 3; j++)
                prev_halo_buffer[i].pos[j] +=
                    v_to_dx * prev_halo_buffer[i].pos[j + 3];
...
void convert_and_sort_core_particles(struct halo *h, struct particle *hp,
                                     float max_r, int64_t *n_core) {
    int64_t i, j, np = h->num_p;
    float   ds, dx;
    for (i = 0; i < np; i++) {
        for (j = 0, dx = 0; j < 3; j++) {
            ds = h->pos[j] - hp[i].pos[j];
            dx += ds * ds;
        }
        hp[i].pos[0] = dx;
...
void reassign_particles_to_parent(struct halo *h, struct particle *hp,
                                  int64_t               *particle_halos,
                                  struct halo           *parent_h,
                                  const struct HaloInfo *haloinfo) {
    int64_t i, prev_id = haloinfo->extra_info[parent_h - haloinfo->halos].ph;
    struct previous_halo *tph = NULL;
    struct inthash       *ih  = NULL;
    if (prev_id < 0)
        return;
    struct fast3tree_results *phtree_res = fast3tree_results_init();
    fast3tree_find_sphere(phtree, phtree_res, parent_h->pos, parent_h->r);
    if (!phtree_res->num_points)
        return;
    ...
    fast3tree_results_free(phtree_res);
    if (!tph)
        return;
    ...
    for (i = 0; i < h->num_p; i++)
        if (ih_getval(ih, p[hp[i].id].id))
            particle_halos[i] = parent_h - haloinfo->halos;

    free_inthash(ih);
    haloinfo->extra_info[h - haloinfo->halos].ph = -1;
}

float find_previous_mass(struct halo *h, struct particle *hp,
                         int64_t *best_num_p, float max_r,
                         const struct HaloInfo *haloinfo) {
    ...
    *best_num_p = 0;
    ...
    qsort(phtree_res->points, phtree_res->num_points,
          sizeof(struct previous_halo *), sort_previous_halos);
    ...
    for (i = 0; i < phtree_res->num_points; i++) {
        ...
        */
        ...
        for (j = 0; j < remaining; j++)
            if (ih_getval(ih, tph->particles[j]))
                cur_part++;
        munlock(tph->particles, tph->num_p * sizeof(int64_t));
    ...
    free_inthash(ih);
    if (best_ph)
        haloinfo->extra_info[h - haloinfo->halos].ph = best_ph->id;
#if DEBUG_FUN_TIMES
    ...
    *best_num_p = best_particles;
    if (best_ph && (best_particles > max_particles * 0.1))
        return best_ph->m;
    return 0;
}
