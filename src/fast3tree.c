...
#define fast3tree_maxmin_rebuild                                               \
    _F3TN(FAST3TREE_PREFIX, fast3tree_maxmin_rebuild)
void fast3tree_maxmin_rebuild(struct fast3tree *t);
...
void fast3tree_find_sphere(struct fast3tree *t, struct fast3tree_results *res,
                           float c[FAST3TREE_DIM], float r);

#undef fast3tree_find_sphere_skip
#define fast3tree_find_sphere_skip                                             \
    _F3TN(FAST3TREE_PREFIX, fast3tree_find_sphere_skip)
void fast3tree_find_sphere_skip(struct fast3tree         *t,
                                struct fast3tree_results *res,
                                FAST3TREE_TYPE *tp, float r);

#undef fast3tree_find_sphere_periodic
#define fast3tree_find_sphere_periodic                                         \
    _F3TN(FAST3TREE_PREFIX, fast3tree_find_sphere_periodic)
int fast3tree_find_sphere_periodic(struct fast3tree         *t,
                                   struct fast3tree_results *res,
                                   float c[FAST3TREE_DIM], float r);

#undef fast3tree_find_sphere_marked
#define fast3tree_find_sphere_marked                                           \
    _F3TN(FAST3TREE_PREFIX, fast3tree_find_sphere_marked)
int fast3tree_find_sphere_marked(struct fast3tree         *t,
                                 struct fast3tree_results *res,
                                 float c[FAST3TREE_DIM], float r, int periodic,
                                 int do_marking);

#undef fast3tree_find_inside_of_box
#define fast3tree_find_inside_of_box                                           \
    _F3TN(FAST3TREE_PREFIX, fast3tree_find_inside_of_box)
void fast3tree_find_inside_of_box(struct fast3tree         *t,
                                  struct fast3tree_results *res,
                                  float b[2 * FAST3TREE_DIM]);

#undef fast3tree_find_outside_of_box
#define fast3tree_find_outside_of_box                                          \
    _F3TN(FAST3TREE_PREFIX, fast3tree_find_outside_of_box)
void fast3tree_find_outside_of_box(struct fast3tree         *t,
                                   struct fast3tree_results *res,
                                   float b[2 * FAST3TREE_DIM]);
...
#define _fast3tree_check_realloc                                               \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_check_realloc)
...
#define _fast3tree_maxmin_rebuild                                              \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_maxmin_rebuild)
void _fast3tree_maxmin_rebuild(struct tree3_node *n);

struct fast3tree *fast3tree_init(int64_t n, FAST3TREE_TYPE *p) {
    struct fast3tree *new = NULL;
    new = _fast3tree_check_realloc(new, sizeof(struct fast3tree),
                                   "Allocating fast3tree.");
    if (!new)
        return 0;
...
int _fast3tree_box_inside_box(struct tree3_node *node,
                              float              b[2 * FAST3TREE_DIM]) {
    int i;
    for (i = 0; i < FAST3TREE_DIM; i++) {
        if (node->max[i] > b[i + FAST3TREE_DIM])
            return 0;
        if (node->min[i] < b[i])
            return 0;
    }
...
int _fast3tree_box_intersect_box(struct tree3_node *node,
                                 float              b[2 * FAST3TREE_DIM]) {
    int i;
    for (i = 0; i < FAST3TREE_DIM; i++) {
        if (node->max[i] < b[i])
            return 0;
        if (node->min[i] > b[i + FAST3TREE_DIM])
            return 0;
    }
    return 1;
}
...
/* Accurate, fast */
int _fast3tree_box_not_intersect_sphere(const struct tree3_node *node,
                                        const float c[FAST3TREE_DIM],
                                        const float r) {
    int         i;
    float       d  = 0, e;
    const float r2 = r * r;
...
#define _fast3tree_box_inside_sphere                                           \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_box_inside_sphere)
int _fast3tree_box_inside_sphere(const struct tree3_node *node,
                                 const float c[FAST3TREE_DIM], const float r) {
    int   i;
    float dx, dx2, dist = 0, r2 = r * r;
    if (fabs(c[0] - node->min[0]) > r)
...
#define _fast3tree_sphere_inside_box                                           \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_sphere_inside_box)
int _fast3tree_sphere_inside_box(const struct tree3_node *node,
                                 const float c[FAST3TREE_DIM], const float r) {
    int i;
    for (i = 0; i < FAST3TREE_DIM; i++) {
        if (c[i] - r < node->min[i])
...
#define _fast3tree_check_results_space                                         \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_check_results_space)
void _fast3tree_check_results_space(const struct tree3_node  *n,
                                    struct fast3tree_results *res) {
    if (res->num_points + n->num_points > res->num_allocated_points) {
        res->num_allocated_points = res->num_points + n->num_points + 1000;
        res->points               = _fast3tree_check_realloc(
            res->points, res->num_allocated_points * sizeof(FAST3TREE_TYPE *),
            "Allocating fast3tree results");
    }
}
...
void _fast3tree_find_sphere(const struct tree3_node  *n,
                            struct fast3tree_results *res,
                            float c[FAST3TREE_DIM], const float r) {
    int64_t i, j;
    float   r2, dist, dx;

    if (_fast3tree_box_not_intersect_sphere(n, c, r))
        return;
#if FAST3TREE_DIM < 6
...
#define _fast3tree_find_sphere_skip                                            \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_sphere_skip)
void _fast3tree_find_sphere_skip(const struct tree3_node  *n,
                                 struct fast3tree_results *res,
                                 float c[FAST3TREE_DIM], const float r,
                                 FAST3TREE_TYPE *tp) {
    int64_t i, j;
...
struct fast3tree_results *fast3tree_results_init(void) {
    struct fast3tree_results *res =
        _fast3tree_check_realloc(NULL, sizeof(struct fast3tree_results),
                                 "Allocating fast3tree results structure.");
    res->points               = NULL;
    res->num_points           = 0;
...
#define _fast3tree_find_sphere_offset                                          \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_sphere_offset)
void _fast3tree_find_sphere_offset(struct tree3_node        *n,
                                   struct fast3tree_results *res,
                                   float                     c[FAST3TREE_DIM],
                                   float                     c2[FAST3TREE_DIM],
                                   float o[FAST3TREE_DIM], const float r,
                                   const int marked, const int do_marking) {
    int64_t i, j;
    float   r2, dist, dx;

    int64_t onlyone = (marked && (n->flags & FAST3TREE_MARKED)) ? 1 : 0;

    if (_fast3tree_box_not_intersect_sphere(n, c2, r * 1.01))
        return;
    if (_fast3tree_box_inside_sphere(n, c2,
                                     r * 0.99)) { /* Entirely inside sphere */
        if (do_marking)
            n->flags |= FAST3TREE_MARKED;
        _fast3tree_check_results_space(n, res);
        if (onlyone) {
    ...
    _fast3tree_find_sphere_offset(n->left, res, c, c2, o, r, marked,
                                  do_marking);
    if (onlyone && (cur_points < res->num_points))
        return;
    _fast3tree_find_sphere_offset(n->right, res, c, c2, o, r, marked,
                                  do_marking);
}

#undef _fast3tree_find_sphere_periodic_dim
#define _fast3tree_find_sphere_periodic_dim                                    \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_sphere_periodic_dim)
void _fast3tree_find_sphere_periodic_dim(
    struct fast3tree *t, struct fast3tree_results *res, float c[FAST3TREE_DIM],
    float c2[FAST3TREE_DIM], float o[FAST3TREE_DIM], float r,
    float dims[FAST3TREE_DIM], int dim, int marked, int do_marking) {
    float c3[FAST3TREE_DIM];
    if (dim < 0) {
        _fast3tree_find_sphere_offset(t->root, res, c, c2, o, r, marked,
                                      do_marking);
        return;
    }
...
int fast3tree_find_sphere_periodic(struct fast3tree         *t,
    ...
    res->num_points = 0;
    _fast3tree_find_sphere_periodic_dim(t, res, c, c, o, r, dims,
                                        FAST3TREE_DIM - 1, 0, 0);
    return 1;
}
...
int fast3tree_find_sphere_marked(struct fast3tree         *t,
    ...
    _fast3tree_find_sphere_periodic_dim(t, res, c, c, o, r, dims,
                                        FAST3TREE_DIM - 1, 1, do_marking);
    return 1;
}

#undef _fast3tree_find_outside_of_box
#define _fast3tree_find_outside_of_box                                         \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_outside_of_box)
void _fast3tree_find_outside_of_box(struct tree3_node        *n,
                                    struct fast3tree_results *res,
                                    float b[2 * FAST3TREE_DIM]) {
    int64_t i, j;
        ...
        if (n->div_dim < 0) {
            ...
            for (i = 0; i < n->num_points; i++) {
                for (j = 0; j < FAST3TREE_DIM; j++) {
                    if (n->points[i].pos[j] < b[j])
                        break;
                    if (n->points[i].pos[j] > b[j + FAST3TREE_DIM])
                        break;
                }
                if (j == FAST3TREE_DIM)
                    continue; // Inside
...
#define _fast3tree_find_inside_of_box                                          \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_inside_of_box)
void _fast3tree_find_inside_of_box(struct tree3_node        *n,
                                   struct fast3tree_results *res,
                                   float b[2 * FAST3TREE_DIM]) {
    int64_t i, j;
    if (!_fast3tree_box_intersect_box(n, b))
...
#define _fast3tree_find_largest_dim                                            \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_largest_dim)
...
int64_t _fast3tree_sort_dim_pos(struct tree3_node *node, float balance) {
...
void _fast3tree_find_minmax(struct tree3_node *node) {
    ...
    for (j = 0; j < FAST3TREE_DIM; j++)
        node->min[j] = node->max[j] = p[0].pos[j];
...
void _fast3tree_split_node(struct fast3tree *t, struct tree3_node *node) {
    ...
    if (num_left == node->num_points ||
        num_left == 0) { // In case all node points are at same spot
        node->div_dim = -1;
        return;
    }
    ...
    if ((t->num_nodes + 2) > t->allocated_nodes) {
        ...
        t->root            = _fast3tree_check_realloc(
            t->root, sizeof(struct tree3_node) * (t->allocated_nodes),
            "Tree nodes");
...
#define _fast3tree_rebuild_pointers                                            \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_rebuild_pointers)
...
void _fast3tree_build(struct fast3tree *t) {
    ...
    t->root            = _fast3tree_check_realloc(
        t->root, sizeof(struct tree3_node) * (t->allocated_nodes),
        "Tree nodes"); // Estimate memory load
    t->num_nodes = 1;

    // Get rid of NaNs / infs
    for (i = 0; i < t->num_points; i++) {
        for (j = 0; j < FAST3TREE_DIM; j++)
            if (!isfinite(t->points[i].pos[j]))
                break;
        if (j < FAST3TREE_DIM) {
            tmp = t->points[i];
            t->num_points--;
    ...
    t->root = _fast3tree_check_realloc(
        t->root, sizeof(struct tree3_node) * (t->num_nodes), "Tree nodes");
...
#define _fast3tree_find_next_closest_dist                                      \
    _F3TN(FAST3TREE_PREFIX, _fast3tree_find_next_closest_dist)
float _fast3tree_find_next_closest_dist(const struct tree3_node *n,
                                        const float c[FAST3TREE_DIM], float r,
                                        const struct tree3_node *o_n,
                                        int64_t                 *counts) {
    int64_t i, j;
    float   r2, dist, dx, *pos;

    if (_fast3tree_box_not_intersect_sphere(n, c, r) || (o_n == n))
        return r;
    if (n->div_dim < 0) { /* Leaf node */
        r2 = r * r;
        for (i = 0; i < n->num_points; i++) {
...
#define fast3tree_find_next_closest_distance                                   \
    _F3TN(FAST3TREE_PREFIX, fast3tree_find_next_closest_distance)
float fast3tree_find_next_closest_distance(struct fast3tree         *t,
                                           struct fast3tree_results *res,
                                           float c[FAST3TREE_DIM]) {
    int64_t            i    = 0, j;
    ...
    while (nd != t->root &&
           (nd->min[nd->parent->div_dim] == nd->max[nd->parent->div_dim]))
        nd = nd->parent;
    ...
    int64_t counts = 0;
    min_dist = _fast3tree_find_next_closest_dist(t->root, c, sqrt(min_dist), nd,
                                                 &counts);
    return min_dist;
}
