...
struct recipient *add_recipient(int64_t struct_size, int64_t c) {
    ...
    num_recipients++;
    recipients =
        check_realloc(recipients, (sizeof(struct recipient) * num_recipients),
...
void clear_particle_rbuffer(struct recipient *r) {
    ...
    send_to_socket_noconfirm(r->cs, r->buffer,
                             sizeof(struct particle) * r->buffered);
    r->buffered = 0;
}
...
void clear_bparticle_rbuffer(struct recipient *r) {
    ...
    send_to_socket_noconfirm(r->cs, r->buffer,
                             sizeof(struct bparticle) * r->buffered);
    r->buffered = 0;
}
...
void add_sp_to_buffer(struct recipient *r, struct sphere_request *sp) {
    assert(r->buffered >= 0);
    if (!(r->buffered % 1000))
        r->buffer = check_realloc(
            r->buffer, sizeof(struct sphere_request) * (r->buffered + 1000),
            "Sphere request buffer");
...
void add_particle_to_buffer(struct recipient *r, struct particle *p1) {
    ...
    if (r->buffered == RECIPIENT_BUFFER)
        clear_particle_rbuffer(r);
...
void check_bparticle_bounds(struct bparticle *tbp, struct recipient *r) {
    ...
    //  if (_check_bounds(tbp->pos, pt.pos, r->bounds))
    //  add_bparticle_to_buffer(r, &pt);
    if (_check_bounds(tbp->pos, pt.pos, r->bounds))
        add_bparticle_to_buffer(r, tbp);
}

void check_halo_bounds(struct halo *h1, struct recipient *r) {
    struct halo ht = *h1;
    if (_check_bounds(h1->pos, ht.pos, r->bounds))
        add_halo_to_buffer(r, &ht);
}
...
void check_bgc2_bounds(struct halo *h1, struct recipient *r) {
    ...
    sp.r = h1->r * BGC2_R;
    if (STRICT_SO_MASSES)
        sp.r = BGC2_R * max_halo_radius(h1);
    for (i = 0; i < 3; i++) {
        bounds[i]     = r->bounds[i] - sp.r;
        bounds[i + 3] = r->bounds[i + 3] + sp.r;
    }
    if (_check_bounds(h1->pos, sp.cen, bounds))
        add_sp_to_buffer(r, &sp);
}
...
#define snd(x)                                                                 \
    {                                                                          \
        data = x;                                                              \
        send_to_socket_noconfirm(c, &data, sizeof(double));                    \
    }
    snd(PARTICLE_MASS);
...
void recv_config(int64_t c) {
    int64_t i;
#define rcv(x)                                                                 \
    { recv_from_socket(c, &x, sizeof(double)); }
    rcv(PARTICLE_MASS);
...
void send_particles(int64_t c, float *bounds) {
    ...
    for (i = num_p - 1; i >= 0; i--) {
        for (j = 0; j < num_recipients; j++)
            if (check_particle_bounds(p + i, recipients + j))
                break;
        if (!(i % PARTICLE_REALLOC_NUM)) {
            p = check_realloc(p, sizeof(struct particle) * i,
                              "Freeing particle memory.");
            check_mtrim();
        }
    }
    num_p = 0;
...
void send_bparticles(char *c_address, char *c_port) {
    ...
    for (i = 0; i < num_bp; i++)
        for (j = 0; j < num_recipients; j++)
            check_bparticle_bounds(bp + i, recipients + j);
...
void gather_spheres(char *c_address, char *c_port, float *bounds,
                    int64_t id_offset, int64_t snap, int64_t chunk) {
    int64_t i, j, k, c;
    ...
    for (i = 0; i < num_halos; i++) {
        ...
        if (STRICT_SO_MASSES)
            r = BGC2_R * max_halo_radius(halos + i);
        for (j = 0; j < 3; j++) {
            if (halos[i].pos[j] - r < bounds[j])
                break;
            if (halos[i].pos[j] + r > bounds[j + 3])
                break;
        }
        if (j == 3)
            continue;
        for (j = 0; j < num_recipients; j++) {
            if (recipients[j].chunk == our_chunk)
                continue;
    ...
    for (j = 0; j < num_recipients; j++) {
        ...
        send_to_socket_noconfirm(c, recipients[j].buffer,
                                 sizeof(struct sphere_request) *
                                     recipients[j].buffered);
        k = 1;
        while (1) {
            ...
            if (!k)
                break;
            ep2 = check_realloc(
                ep2, sizeof(struct extended_particle) * (num_ep2 + k),
                "Allocating secondary extended particles.");
            recv_from_socket(c, ep2 + num_ep2,
                             sizeof(struct extended_particle) * k);
            num_ep2 += k;
        }
...
void collect_bgroups(int64_t chunk) {
    ...
    bgroups_to_setlist();
    clear_bg_data();
    for (i = 0, total_bg = 0; i < num_bg_sets; i++)
    ...
    while (num_bg_sets && total_bg) {
        ...
        send_to_socket_noconfirm(c, bg_set_sizes,
                                 sizeof(int64_t) * num_bg_sets);
        for (i = 0, total_bg = 0; i < num_bg_sets; i++)
            total_bg += bg_set_sizes[i];
        ...
        if (num_bg_sets) {
            bg_set_sizes =
                recv_and_alloc(c, bg_set_sizes, sizeof(int64_t) * num_bg_sets);
    ...
    for (i = 0; i < NUM_WRITERS; i++)
        sets_per_chunk[i] = chunk_indices[i] = 0;
    ...
    for (i = 0; i < num_bg_sets; i++) {
        ...
        qsort(final_bg + j, bg_set_sizes[i], sizeof(struct bgroup),
              sort_by_chunk);
    ...
    for (connect_chunk = 0; connect_chunk < NUM_WRITERS; connect_chunk++) {
        if (!sets_per_chunk[connect_chunk])
            continue;
        c = connect_to_addr(chunk_info[connect_chunk].address,
                            chunk_info[connect_chunk].port);
        send_to_socket_noconfirm(c, "strb", 4);
        send_to_socket_noconfirm(c, &chunk, sizeof(int64_t));
        send_to_socket_noconfirm(c, sets_per_chunk + connect_chunk,
                                 sizeof(int64_t));
        ...
        send_to_socket_noconfirm(c, bg_set_sizes + chunk_indices[connect_chunk],
                                 sizeof(int64_t) *
                                     sets_per_chunk[connect_chunk]);
        for (j = 0, total_bg = 0; j < sets_per_chunk[connect_chunk]; j++)
            total_bg += bg_set_sizes[chunk_indices[connect_chunk] + j];
        send_to_socket_noconfirm(c, final_bg + chunk_offset,
                                 total_bg * sizeof(struct bgroup));
        chunk_offset += total_bg;
        send_to_socket(c, "done", 4);
        close_rsocket(c);
    }

    c = connect_to_addr(chunk_info[chunk].address, chunk_info[chunk].port);
    send_to_socket(c, "rdne", 4);
    close_rsocket(c);
    exit(0);
}
...
void send_halos(char *c_address, char *c_port, int64_t snap, int64_t chunk) {
    ...
    for (i = 0; i < bheader.num_halos; i++)
        for (j = 0; j < num_recipients; j++)
            check_halo_bounds(halos + i, recipients + j);
...
void transfer_stuff(int64_t s, int64_t c, int64_t timestep) {
    int64_t i, j, k, num_senders = 0, done = 0, length, new_p_start,
                     *senders           = NULL;
    char                         cmd[5] = {0};
    struct binary_output_header *bheader;
    ...
    while (!in_error_state && (num_senders || !done)) {
        ...
        for (i = 0; i < num_senders; i++)
            tag_rsocket(senders[i]);
        max_conn = select_rsocket(RSOCKET_READ, 0);
        for (i = 0; i < max_conn; i++) {
            if (!check_rsocket_tag(i))
                continue;
            if (i == s) {
                num_senders++;
                ...
                if (!strcmp(cmd, "done")) {
                    ...
                    for (j = 0; j < num_senders; j++)
                        close_rsocket(senders[j]);
                    num_senders = 0;
                    break;
                } else {
                    fprintf(stderr, "[Error] Server protocol error rs (%s)!\n",
                            cmd);
                    exit(1);
                }
            } else {
                ...
                if (!strcmp(cmd, "part")) {
                    ...
                    if (!epbuffer)
                        epbuffer =
                            check_realloc(NULL,
                                          sizeof(struct extended_particle) *
                                              PARTICLE_REALLOC_NUM,
                                          "Particle buffer");
                    if (!bitarray)
                        bitarray = BIT_ALLOC(num_p + num_additional_p);
                    ...
                    for (j = 0; j < length; j++) {
                        ...
                        for (l = 0; l < num_sp; l++) {
                            ...
                            if (k == PARTICLE_REALLOC_NUM) {
                                send_to_socket_noconfirm(i, &k,
                                                         sizeof(int64_t));
                                send_to_socket_noconfirm(
                                    i, epbuffer,
                                    sizeof(struct extended_particle) * k);
                                k = 0;
                            }
                    ...
                    if (k) {
                        ...
                        send_to_socket_noconfirm(
                            i, epbuffer, sizeof(struct extended_particle) * k);
                        k = 0;
                    }
                    send_to_socket_noconfirm(i, &k, sizeof(int64_t));
                    free(sp);
                    ...
                    *pids_recv = recv_msg(i, *pids_recv, &length, length);
                    ...
                    if (!strcmp(cmd, "lnkb")) {
                        if (num_sets) {
                            set_sizes = recv_and_alloc(
                                i, NULL, sizeof(int64_t) * num_sets);
                            for (j = 0, total_bg = 0; j < num_sets; j++)
                                total_bg += set_sizes[j];
                            bgroup_request = recv_and_alloc(
                                i, NULL, sizeof(struct bgroup) * total_bg);
                        }
                        ...
                        if (num_sets) {
                            send_to_socket_noconfirm(
                                i, set_sizes, sizeof(int64_t) * num_sets);
                            send_to_socket_noconfirm(i, bgroup_request,
                                                     sizeof(struct bgroup) *
                                                         total_groups);
                            free(set_sizes);
                            free(bgroup_request);
                        }
                    } else if (num_sets) {
                        check_realloc_s(bg_set_sizes, sizeof(int64_t),
                                        num_bg_sets + num_sets);
                        recv_from_socket(i, bg_set_sizes + num_bg_sets,
                                         sizeof(int64_t) * num_sets);
                        for (j = 0, total_bg = 0; j < num_bg_sets + num_sets;
                             j++)
                            total_bg += bg_set_sizes[j];
                        check_realloc_s(final_bg, sizeof(struct bgroup),
                                        total_bg);
                        bgroup_request = final_bg + total_bg;
                        for (j = 0, total_bg = 0; j < num_sets; j++)
                            total_bg += bg_set_sizes[j + num_bg_sets];
                        bgroup_request -= total_bg;
                        recv_from_socket(i, bgroup_request,
                                         sizeof(struct bgroup) * total_bg);
                        num_bg_sets += num_sets;
                    }
                }
                    ...
                    fprintf(stderr, "[Error] Client protocol error rs (%s)!\n",
                            cmd);
                    exit(1);
                }
            }
    ...
    for (j = 0; j < num_p; j++) {
        for (i = 0; i < num_proj; i++) {
            if (check_projection_bounds(p + j, prj + i)) {
                idx =
                    (double)PROJECTION_SIZE * p[j].pos[dir] / (double)BOX_SIZE;
                if (idx >= PROJECTION_SIZE)
                    idx = PROJECTION_SIZE - 1;
                prj[i].data[idx]++;
            }
        }
    }
...
void accept_workloads(char *c_address, char *c_port, int64_t snap,
                      int64_t chunk) {
    ...
    int64_t idle_time = 0, recv_time = 0, send_time = 0, bp_time = 0,
            f_time = 0, work_time = 0, ph_time = 0, total_pp = 0, total_h = 0,
            total_wku = 0;
...
#define record_time(x)                                                         \
    ...
    while (1) {
        if (recv_from_socket(m, cmd, 4) <= 0) {
            fprintf(stderr,
                    "[Warning] Failed to receive instruction from server for "
                    "chunk %" PRId64 " (shutting down)!\n",
                    chunk);
        ...
        if (!strcmp(cmd, "wrku")) {
            ...
            assert((w.num_particles + w.num_meta_p) >= 0 && w.num_fofs >= 0 &&
                   !w.num_halos);
            ...
            if (w.num_particles)
                recv_from_socket(m, p,
                                 sizeof(struct particle) * w.num_particles);
            record_time(recv_time);

            if (w.num_meta_fofs) {
                set_sizes   = recv_and_alloc(m, set_sizes,
                                             sizeof(int64_t) * w.num_meta_fofs);
                bgroup_list = recv_and_alloc(
                    m, bgroup_list, sizeof(struct bgroup) * w.total_bg);
                loc = 0;
                for (i = 0; i < w.num_meta_fofs; i++) {
                    qsort(bgroup_list + loc, set_sizes[i],
                          sizeof(struct bgroup), sort_by_chunk);
                    loc += set_sizes[i];
                }
                ...
                qsort(bgroup_list, w.total_bg, sizeof(struct bgroup),
                      sort_by_chunk);
                int64_t dup_ids = 0;
                for (i = 1; i < w.total_bg; i++)
                    if (bgroup_list[i].chunk == bgroup_list[i - 1].chunk &&
                        bgroup_list[i].id == bgroup_list[i - 1].id)
                        dup_ids = 1;
                ...
                while (i < w.total_bg) {
                    ...
                    for (; i < w.total_bg; i++) {
                        if (bgroup_list[loc].chunk != bgroup_list[i].chunk)
                            break;
                        part_in_chunk += bgroup_list[i].num_p;
                    }
                    int64_t num_groups = i - loc;
                    if (!num_groups || !part_in_chunk)
                        break;
                    ...
                    send_to_socket_noconfirm(m2, bgroup_list + loc,
                                             sizeof(struct bgroup) *
                                                 num_groups);
                    recv_from_socket(m2, chunk_info[cchunk].bounds,
                                     sizeof(float) * 6);
                ...
                if (memcmp(w.bounds, zero_bounds, sizeof(float) * 6) != 0) {
                    if (!PERIODIC || !BOX_SIZE)
                        ...
                        calc_particle_bounds_periodic(w.bounds);
                }
                new_bounds = 1;
                assert(!dup_ids);
            } else {
            ...
            if (CLIENT_DEBUG)
                fprintf(stderr,
                        "Received %" PRId64 " particles and %" PRId64
                        " fofs from id %" PRId64 " (Worker %" PRId64 ")\n",
                        num_p, w.num_fofs, id - NUM_READERS, chunk);
            if (new_bounds && TEMPORAL_HALO_FINDING) {
                new_bounds = 0;
                if (!memcmp(w.bounds, zero_bounds, sizeof(float) * 6))
                    load_previous_halos(snap, w.chunk, NULL); // Single
                                                              // processor
                else
                    load_previous_halos(snap, w.chunk, w.bounds);
            }
            ...
            if (CLIENT_DEBUG)
                fprintf(stderr,
                        "Analyzed %" PRId64 " particles and %" PRId64
                        " fofs from id %" PRId64 ", and found %" PRId64
                        " halos. (Worker %" PRId64 ")\n",
                        w.num_particles, w.num_fofs, id - NUM_READERS,
                        num_halos, chunk);
            ...
            //      fprintf(stderr, "Workunit done (%"PRId64" halos found;
            ...
            for (i = 0; i < num_chunks; i++) {
                ...
                sort_out_halos_for_chunk(
                    chunks[i], chunk_info[chunks[i]].bounds, &chunk_w,
                    &chunk_fofs, &chunk_halos, &chunk_ei, &chunk_p, fofs);
                if (chunk_w.num_halos) {
                    ...
                    send_to_socket_noconfirm(m2, &chunk_w,
                                             sizeof(struct workunit_info));
                    send_to_socket_noconfirm(
                        m2, chunk_fofs, sizeof(struct fof) * chunk_w.num_fofs);
                    send_to_socket_noconfirm(m2, chunk_halos,
                                             sizeof(struct halo) *
                                                 chunk_w.num_halos);
                    send_to_socket_noconfirm(m2, chunk_ei,
                                             sizeof(struct extra_halo_info) *
                                                 chunk_w.num_halos);
                    send_to_socket(
                        m2, chunk_p,
                        sizeof(struct particle) *
                            (chunk_w.num_particles + chunk_w.num_meta_p));
                    close_rsocket(m2);
            ...
            if (profile_out)
                fprintf(profile_out,
                        "[Prof] S%" PRId64 ",C%" PRId64 ": %" PRId64
                        "p,%" PRId64 "h,%" PRId64 "w; wt:%" PRId64
                        "s; rcv:%" PRId64 "s,%" PRId64 "s; snd:%" PRId64
                        "s; wk:%" PRId64 "s; idl:%" PRId64 "s\n",
                        snap, chunk, total_pp, total_h, total_wku, idle_time,
                        recv_time, bp_time, send_time, work_time, f_time);
            fflush(profile_out);
            exit(0);
        } else {
...
int send_workunit(int64_t sock, struct workunit_info *w, struct fof **fofs,
                  struct particle **particles, int64_t **set_sizes,
                  struct bgroup **bgroup_list, int64_t *no_more_work,
    ...
    if (bounds)
        ...
        memset(w->bounds, 0, sizeof(float) * 6);
    w->chunk = chunk;
    ...
    if ((*particles >= p) && (*particles < p + num_p))
        *particles = NULL;
    return 1;
}

int64_t distribute_workloads(int64_t c, int64_t s, int64_t snap, int64_t chunk,
                             float *bounds) {
    int64_t i, j, num_workers = 0, no_more_work = 0, workdone = 0,
                  all_clear = 0, done = 0, id_offset = 0, worker_chunk, hcnt,
                  id;
    int64_t *workers = NULL, max_cs = 0, child = -1, new_w,
            child_has_connected     = 0;
    char                   *address = NULL, *port = NULL;
    char                    cmd[5] = {0};
    struct workunit_info    w;
    ...
    while ((num_workers || !done) && !(in_error_state)) {
        ...
        if (all_clear && !workdone && no_more_work && child_has_connected) {
            ...
            if (CLIENT_DEBUG)
                fprintf(stderr,
                        "Analysis of %" PRId64 " halos complete (chunk %" PRId64
                        ")\n",
                        hcnt, chunk);
        }
            ...
            if (i == s) {
                ...
                if (worker_chunk == -1) {
                    ...
                    if (!strcmp(cmd, "part")) {
                        ...
                        bgroup_list = recv_and_alloc(
                            new_w, NULL, num_groups * sizeof(struct bgroup));
                        send_to_socket(new_w, bounds, sizeof(float) * 6);
                        if (!pbuffer)
                            pbuffer = check_realloc(NULL,
                                                    sizeof(struct particle) *
                                                        PARTICLE_REALLOC_NUM,
                                                    "Particle buffer");
                        int64_t    k = 0, l = 0;
                        ...
                        if (l)
                            send_to_socket(new_w, pbuffer,
                                           sizeof(struct particle) * l);
                        if (bgroup_list)
                            bgroup_list = check_realloc(bgroup_list, 0,
                                                        "Freeing bgroups");
                    } else if (!strcmp(cmd, "wrkd")) {
                        recv_from_socket(new_w, &w,
                                         sizeof(struct workunit_info));
                        assert((w.num_particles + w.num_meta_p) >= 0);
                        fofs   = recv_and_alloc(new_w, fofs,
                                                sizeof(struct fof) * w.num_fofs);
                        rhalos = recv_and_alloc(
                            new_w, rhalos, sizeof(struct halo) * w.num_halos);
                        ehi   = recv_and_alloc(new_w, ehi,
                                               sizeof(struct extra_halo_info) *
                                                   w.num_halos);
                        parts = recv_and_alloc(
                            new_w, parts,
                            sizeof(struct particle) *
                                (w.num_particles + w.num_meta_p));
                        integrate_finished_workunit(&w, fofs, rhalos, ehi,
                                                    parts);
                    }
                    close_rsocket(new_w);
                } else {
                    if (send_workunit(new_w, &w, &fofs, &parts, &set_sizes,
                                      &bgroup_list, &no_more_work, chunk,
                                      bounds) ||
                        (child == new_w)) {
                        workers = check_realloc(
                            workers, sizeof(int64_t) * (num_workers + 1),
                            "Allocating rockstar analysis FDs.");
                        workers[num_workers] = new_w;
                        num_workers++;
                        if (CLIENT_DEBUG)
                            fprintf(stderr,
                                    "Got new worker (chunk %" PRId64
                                    "; wchunk %" PRId64 ")\n",
                                    chunk, worker_chunk);
                    } else {
                        close_rsocket(new_w);
                ...
                if (!strcmp(cmd, "fini")) {
                    ...
                    if (CLIENT_DEBUG)
                        fprintf(stderr,
                                "Told child to finish (chunk %" PRId64 ")\n",
                                chunk);
                } else if (!strcmp(cmd, "work")) {
                    assert(child >= 0);
                    ...
                    if (CLIENT_DEBUG)
                        fprintf(stderr,
                                "Child (%" PRId64 ") connecting to id %" PRId64
                                "\n",
                                chunk, id);
                } else if (!strcmp(cmd, "allc"))
                    all_clear = 1;
                    ...
                    if (CLIENT_DEBUG)
                        fprintf(stderr, "Finished (chunk %" PRId64 ")\n",
                                chunk);
                    ...
                    fprintf(stderr, "[Error] Server protocol error dw (%s)!\n",
                            cmd);
                    exit(1);
                }
            } else {
                ...
                if (!strcmp(cmd, "wrku")) {
                    if (!send_workunit(i, &w, &fofs, &parts, &set_sizes,
                                       &bgroup_list, &no_more_work, chunk,
                                       bounds) &&
                        (child != i))
                    ...
                    fprintf(stderr, "[Error] Client protocol error dw (%s)!\n",
                            cmd);
                    exit(1);
                }
            }
    ...
    if (pbuffer)
        check_realloc(pbuffer, 0, "Freeing particle buffer");
    if (set_sizes)
        check_realloc(set_sizes, 0, "Freeing set sizes");
    if (bgroup_list)
        check_realloc(bgroup_list, 0, "Freeing bgroups");
    return id_offset;
}
...
void client(int64_t type) {
    ...
    if (FORK_READERS_FROM_WRITERS) {
        ...
        for (i = 0; i < readers; i++) {
            ...
            if (n < 0)
                system_error("Couldn't fork reader process!");
        }
        if (i == readers) {
            type = WRITER_TYPE;
        }
    ...
    if (FORK_PROCESSORS_PER_MACHINE && (type != READER_TYPE)) {
        for (i = 1; i < FORK_PROCESSORS_PER_MACHINE; i++) {
            ...
            if (n == 0)
                break;
            if (n < 0)
                system_error("Couldn't fork process!");
        }
    }
    ...
    if (magic != ROCKSTAR_MAGIC) {
        fprintf(stderr, "[Error] Received invalid client responses.  Check "
                        "network connectivity.\n");
        exit(1);
    }
    send_to_socket_noconfirm(c, &magic, sizeof(uint64_t));
    if (type < 0)
        send_to_socket_noconfirm(c, "rdwr", 4);
    if (type == READER_TYPE)
    ...
    while (1) {
        ...
        if (!strcmp(cmd, "read")) {
            ...
            if (i >= 10000) {
                fprintf(stderr,
                        "[Error] Couldn't start particle data server at "
                        "%s:%d-%d!\n",
            ...
            if (LIGHTCONE && strlen(LIGHTCONE_ALT_SNAPS)) {
                memcpy(LIGHTCONE_ORIGIN, LIGHTCONE_ALT_ORIGIN,
                       sizeof(double) * 3);
            }
        }
            ...
            if (chunk_info) {
                free(chunk_info[0].address);
                free(chunk_info);
            }
            chunk_info =
                check_realloc(NULL, sizeof(struct chunk_info) * NUM_WRITERS,
                              "Allocating address information");
            chunk_info[0].address = recv_msg_nolength(c, NULL);
            chunk_info[0].port =
                chunk_info[0].address + strlen(chunk_info[0].address) + 1;
            for (i = 1; i < NUM_WRITERS; i++) {
                chunk_info[i].address =
                    chunk_info[i - 1].port + strlen(chunk_info[i - 1].port) + 1;
                chunk_info[i].port =
                    chunk_info[i].address + strlen(chunk_info[i].address) + 1;
            }
        }
            ...
            if (num_proj) {
                recv_from_socket(c, prq,
                                 sizeof(struct projection_request) * num_proj);
                if (CLIENT_DEBUG)
                    fprintf(stderr,
                            "Doing client projections for block %" PRId64 "\n",
                            block);
                do_projections();
                if (CLIENT_DEBUG)
                    fprintf(stderr,
                            "Done with client projections for block %" PRId64
                            "\n",
                            block);
            }
            ...
            if (profile_out) {
                fprintf(profile_out,
                        "[Prof] S%" PRId64 ",C%" PRId64 " %" PRId64
                        "s: %" PRId64 " fofs, %" PRId64 " particles, %" PRId64
                        "s for conf.\n",
                        snap, chunk, (time_middle - time_start), num_all_fofs,
            ...
            n = fork();
            if (!n)
                accept_workloads(hostname, port, snap, chunk);
            if (n < 0)
                system_error("Couldn't fork halo analysis process!");
            ...
            if (!p_bounds)
                check_realloc_s(p_bounds, sizeof(struct prev_bounds),
                                NUM_WRITERS);
            recv_from_socket(c, p_bounds,
            ...
            n = fork();
            if (!n)
                send_halos(hostname, port, snap, chunk);
            if (n < 0)
                system_error("Couldn't fork merger tree process!");
            ...
            n          = fork();
            if (!n)
                send_bparticles(hostname, port);
            if (n < 0)
                system_error("Couldn't fork boundary particle process!");
            transfer_stuff(s, c, 0);
            check_waitpid(n);
            if (in_error_state) {
                network_error_cleanup();
            ...
            n = fork();
            if (!n)
                collect_bgroups(chunk);
            if (n < 0)
                system_error("Couldn't fork boundary group process!");
            transfer_stuff(s, c, 0);
            check_waitpid(n);
            if (in_error_state) {
                network_error_cleanup();
            ...
            n = fork();
            if (!n)
                gather_spheres(hostname, port, bounds, id_offset, snap, chunk);
            if (n < 0)
                system_error("Couldn't fork boundary group process!");
            transfer_stuff(s, c, 0);
            check_waitpid(n);
            ...
            snprintf(buffer, 1024,
                     "%s %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " '%s'",
                     RUN_PARALLEL_ON_SUCCESS, snap, NUM_SNAPS, chunk,
                     NUM_WRITERS, ROCKSTAR_CONFIG_FILENAME);
            if (system(buffer) != 0) {
                system_error(
                    "Running external parallel analysis process failed.");
