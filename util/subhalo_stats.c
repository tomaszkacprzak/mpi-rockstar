...
void decompose_v(float *pos, float *axis, float *vel, float *vr, float *vtheta,
                 float *vphi) {
    float theta[3], phi[3];
    float tnorm, pnorm;
...
void _calc_subhalo_stats(struct halo *h, FILE *output) {
    ...
    while (parent->parent >= 0)
        parent = halos + parent->parent;
    if (!parent->m || !parent->r || !parent->vmax || !parent->rs)
        return;
    ...
    fprintf(output,
            "%" PRId64 " %" PRId64 " %.3e %.3e %.3f %.3f %.3f %.3f %.1f "
            "%.3f %.3f %.3f %.3f %.3f\n",
            h->id, parent->id, h->m / parent->m, parent->m,
            h->vmax / parent->vmax, parent->vmax, h->r / parent->r,
...
void calc_subhalo_stats(int64_t snap) {
    ...
    for (i = 0; i < NUM_WRITERS; i++) {
        ...
        halos = check_realloc(halos,
                              sizeof(struct halo) * (num_halos + hdr.num_halos),
                              "Allocating room for halos.");
        check_fread(halos + num_halos, sizeof(struct halo), hdr.num_halos,
                    input);
    ...
    fprintf(output, "#ID UPID Mass/Pmass Pmass Vmax/Pvmax Pvmax Rvir/Prvir "
                    "Prvir Pc R/Rvir Theta Vr/Pvmax Vtheta/Pvmax Vphi/Pvmax\n");
