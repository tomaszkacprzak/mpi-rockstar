...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < num_groups; i++) {
        printf("%" PRId64 " -1 %.5g %.3f 0 %.3f 0 %" PRId64
               " %f %f %f %f %f %f %" PRId64 "\n",
               gd[i].id, gd[i].mass, gd[i].vmax, gd[i].radius * 1.0e3,
               gd[i].npart, gd[i].pos[0], gd[i].pos[1], gd[i].pos[2],
               gd[i].vel[0], gd[i].vel[1], gd[i].vel[2], gd[i].parent_id);
    }
    return 0;
}
