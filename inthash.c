...
struct inthash *new_inthash(void) {
    struct inthash *ih =
        check_realloc(NULL, sizeof(struct inthash), "Allocating inthash.");
    int64_t i;
    memset(ih, 0, sizeof(struct inthash));
    ih->hashnum =
        rand() + (((uint64_t)rand()) << (uint64_t)32) + (uint64_t)(rand());
    ih->hashwidth   = 8;
    ih->num_buckets = (uint64_t)1 << ih->hashwidth;
    ih->buckets =
        check_realloc(NULL, sizeof(struct intbucket) * ih->num_buckets,
                      "Allocating hash buckets.");
    memset(ih->buckets, 0, sizeof(struct intbucket) * ih->num_buckets);
    for (i = 0; i < ih->num_buckets; i++)
        ih->buckets[i].key = IH_INVALID;
    if (!(ih->hashnum & 1))
        ih->hashnum++;
    return ih;
}
...
int64_t *ih_keylist(struct inthash *ih) {
    int64_t           i, j = 0;
    int64_t          *kl = check_realloc(NULL, sizeof(int64_t) * ih->elems,
                                         "Allocating key list.");
...
uint64_t _ih_hash_function(struct inthash *ih, uint64_t key) {
...
void _ih_add_more_buckets(struct inthash *ih, int64_t add_factor) {
    ...
    new_buckets =
        check_realloc(NULL, new_alloc_size, "Allocating new buckets.");
    memset(new_buckets, 0, new_alloc_size);
    for (i = 0; i < ih->num_buckets; i++)
        new_buckets[i].key = IH_INVALID;
...
void ih_prealloc(struct inthash *ih, int64_t size) {
    ...
    if (numbits <= ih->hashwidth)
        return;
    _ih_add_more_buckets(ih, numbits - ih->hashwidth);
}

struct intbucket *_ih_getval(struct inthash *ih, int64_t key) {
...
struct intbucket *_ih_getval_deleted(struct inthash *ih, int64_t key) {
    struct intbucket *ib     = ih->buckets + _ih_hash_function(ih, key);
...
void free_inthash2(struct inthash *ih) {
    ...
    for (i = 0; i < ih->num_buckets; i++)
        if (ih->buckets[i].key)
            free_inthash(ih->buckets[i].data);
    free_inthash(ih);
}
