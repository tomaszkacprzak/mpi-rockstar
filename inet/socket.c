...
int _connect_to_addr(char *host, char *port) {
    ...
    if (s < 0) {
        fprintf(stderr,
                "[Error] Couldn't open socket for address %s:%s!  (Err: %s)\n",
    ...
    for (i = 0; i < RETRIES; i++) {
        ...
        snprintf(buffer, 200,
                 "[Warning] Connection attempt %d to %s:%s failed: ", i + 1,
                 host, port);
        perror(buffer);
        random_sleep(10);
    }
    if (i == RETRIES) {
        fprintf(stderr, "[Error] Failed to connect to %s:%s! (Err: %s;\n", host,
                port, strerror(errno));
        fprintf(stderr,
                " This error may mean that the connection was refused.)\n");
        exit(1);
    }
    freeaddrinfo(res);
    return s;
...
int _accept_connection(int s, char **address, int *port) {
    ...
    if (peer.sockaddr_stor.ss_family == AF_INET) {
        if (port)
            *port = ntohs(peer.sockaddr_in.sin_port);
        if (address && *address)
            inet_ntop(AF_INET, &(peer.sockaddr_in.sin_addr), *address, size);
    } else { // Assume AF_INET6
        if (port)
            *port = ntohs(peer.sockaddr_in6.sin6_port);
        if (address && *address)
            inet_ntop(AF_INET6, &peer.sockaddr_in6.sin6_addr, *address, size);
    }
    return a;
}
...
void *_recv_msg(int s, void *data, int64_t *length, int64_t offset) {
    int64_t incoming;
    if (_recv_from_socket(s, &incoming, sizeof(int64_t)) < 0)
        return data;
