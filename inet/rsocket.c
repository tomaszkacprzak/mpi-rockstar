...
int64_t check_rsocket_tag(int64_t s) {
    if (s > num_rsockets || s < 0)
        rsocket_fatal("Attempted use of invalid socket.");
    if (rsockets[s].flags & UNUSED_FLAG)
        return 0;
...
int64_t select_rsocket(int poll_type, double timeout) {
    ...
    if (poll_alloced != num_rsockets) {
        ...
        rsocket_poll = socket_check_realloc(
            rsocket_poll, sizeof(struct pollfd) * (num_rsockets + 1),
            "Allocating poll fds.");
    }
    if (poll_type & RSOCKET_READ)
        poll_events |= POLLIN;
    if (poll_type & RSOCKET_WRITE)
        poll_events |= POLLOUT;
    if (poll_type & RSOCKET_ERROR)
        poll_events |= POLLERR;
    ...
    for (i = 0; i < num_rsockets; i++) {
        if ((rsockets[i].flags & (SELECTED_FLAG | META_SELECTED_FLAG)) &&
            !(rsockets[i].flags & (UNUSED_FLAG))) {
            if (rsockets[i].fd > max_fd)
                max_fd = rsockets[i].fd;
    ...
    while ((res = poll(rsocket_poll, nfds,
                       (timeout > 0.0) ? timeout * 1000 : -1)) < 0 &&
           (errno == EINTR))
        ;
    if (res < 0) {
        ...
        if (rsocket_poll[j].revents & poll_events) {
            if ((rsockets[i].flags & SERVER_FLAG) &&
                ...
                if (rsocket_poll[j].revents & POLLHUP) // Closed:
                    if (recv(rsockets[i].fd, &res, 1, MSG_PEEK) <=
                        0)        // Test for data
...
int64_t add_rsocket(struct rsocket s) {
    ...
    for (s.id = 0; s.id < num_rsockets; s.id++) {
        if (rsockets[s.id].flags & UNUSED_FLAG)
            u = s.id;
        if (s.magic && (rsockets[s.id].magic == s.magic) &&
            (rsockets[s.id].flags & RECEIVER_FLAG)) {
            if (!(rsockets[s.id].flags & UNUSED_FLAG)) {
                if (rsockets[s.id].address != s.address)
                    free(rsockets[s.id].address);
                if (rsockets[s.id].port != s.port)
                    free(rsockets[s.id].port);
    ...
    rsockets               = socket_check_realloc(rsockets,
                                                  sizeof(struct rsocket) * (num_rsockets + 1),
                                                  "Adding new rsocket.");
    s.id                   = num_rsockets;
...
void _reconnect_to_addr(struct rsocket *n) {
    ...
    for (i = 0; i < RETRIES; i++) {
        if (i) {
            random_sleep(i);
            fprintf(stderr,
                    "[Network] Reconnect attempt %" PRId64 " to %s:%s\n", i,
                    n->address, n->port);
        }
        ...
        if ((_send_to_socket(n->fd, &(magic), sizeof(uint64_t)) <= 0) ||
            ...
            fprintf(stderr,
                    "[Network] Failed to send magic number to %s:%s during "
                    "reconnection attempt %" PRId64 "\n",
                    n->address, n->port, i);
            close(n->fd);
            continue;
        }
        ...
        if (_recv_from_socket(n->fd, &magic, sizeof(uint64_t)) <= 0) {
            fprintf(stderr,
                    "[Network] Failed to receive magic number from %s:%s "
                    "during reconnection attempt %" PRId64 "\n",
                    n->address, n->port, i);
            close(n->fd);
            continue;
        }
    ...
    if (!n->magic)
        n->magic = magic;
    else if (n->magic != magic)
        rsocket_fatal("Couldn't reconnect to address!");
}
...
int64_t rsocket_accept_connection(int64_t s, char **address, int *port,
                                  uint64_t desired_magic, int64_t justone) {
    struct rsocket *ts              = rsocket_verify_id(s);
    char            port_buffer[20] = {0};
    int             our_port;
    uint64_t        magic;
    if (!(ts->flags & SERVER_FLAG))
        rsocket_fatal("Not a server socket!");
    while (1) {
        struct rsocket nc = {0};
        ts                = rsocket_verify_id(s);
        nc.fd = _accept_connection(ts->fd, &(nc.address), &our_port);
        if ((_recv_from_socket(nc.fd, &(magic), sizeof(uint64_t)) !=
             sizeof(uint64_t)) ||
            (magic != RSOCKET_VERIFY) ||
            (_recv_from_socket(nc.fd, &(nc.magic), sizeof(uint64_t)) !=
             sizeof(uint64_t))) {
        ...
        if (address && (address != &(nc.address)))
            *address = strdup(nc.address);
        if (port)
            *port = our_port;
...
#ifdef DEBUG_RSOCKET
        fprintf(stderr,
                "Accepted connection %" PRId64 " with sseq %" PRId64
                " rseq %" PRId64 "\n",
                id, rsockets[id].sseq, rsockets[id].rseq);
#endif /*DEBUG_RSOCKET */
        if (!rsockets[id].magic)
            rsockets[id].magic = gen_magic();
        _send_to_socket(nc.fd, &(rsockets[id].magic), sizeof(uint64_t));
        if (justone && !nc.magic)
            rsockets[id].flags |= NEW_FLAG;
        if (nc.magic == desired_magic || justone)
            return id;
        if (!nc.magic)
            rsockets[id].flags |= NEW_FLAG;
    }
...
int64_t send_to_rsocket(int64_t s, void *data, int64_t length, int64_t flags) {
    ...
    while (1) {
        ...
        if (((flags & RPACKET_CONFIRM_SENT_FLAG) ||
             ((_send_to_socket(rs->fd, &(rs->magic), sizeof(uint64_t)) ==
               sizeof(uint64_t)) &&
            ...
            ((flags & (RPACKET_NO_CONFIRM_FLAG | RPACKET_DELAY_CONFIRM_FLAG)) ||
             ((_recv_from_socket(rs->fd, &confirm, sizeof(int64_t)) ==
               sizeof(int64_t)) &&
              (confirm == 1)))) {
            if (!(flags & RPACKET_DELAY_CONFIRM_FLAG)) {
#ifdef DEBUG_RSOCKET
                fprintf(stderr,
                        "[Sent] %" PRId64 " bytes to socket %" PRId64
                        ", seq %" PRId64 ", seqnow %" PRId64 ". ",
                        rp.length, s, rp.seq, rs->sseq);
                if (length < 100)
                    fwrite(data, 1, rp.length, stderr);
                fprintf(stderr, "\n");
#endif /*DEBUG_RSOCKET*/
                rs->sseq++;
...
int64_t recv_from_rsocket(int64_t s, void *data, int64_t length, int64_t offset,
                          int64_t alloc) {
    ...
    while (1) {
        retry++;
        if ((_recv_from_socket(rs->fd, &magic, sizeof(uint64_t)) ==
             sizeof(uint64_t)) &&
            (magic == rs->magic) &&
            ...
            if (rp.seq < rs->rseq && rs->rseq < (INT64_MAX - 10)) {
                fprintf(stderr,
                        "[Warning] Ignoring duplicate sequence %" PRId64
                        " (seqnow: %" PRId64 ")\n",
                        rp.seq, rs->rseq);
                while (lcount < rp.length) {
                    ...
                    if (recvd != to_read) {
                        fprintf(stderr,
                                "[Warning] Incomplete packet received!\n");
                        break;
                    }
                    lcount += recvd;
                }
                ...
                if (alloc) {
                    if (rp.length + offset > length) {
                        ...
                        data   = socket_check_realloc(data, length,
                                                      "receive buffer");
                    }
                } else if ((length != rp.length + offset)) {
                    fprintf(stderr,
                            "Expected receive length (%" PRId64
                            ") != actual receive length (%" PRId64 ")\n",
                            length, rp.length + offset);
                    if (length < 100) {
                ...
                if (_recv_from_socket(rs->fd, ((char *)data) + offset,
                                      rp.length) == rp.length) {
                    if (!(rp.flags & RPACKET_NO_CONFIRM_FLAG))
                        _send_to_socket(rs->fd, &confirm, sizeof(int64_t));
                    rs->last_data = data;
#ifdef DEBUG_RSOCKET
                    fprintf(stderr,
                            "[Received] %" PRId64 " bytes from socket %" PRId64
                            ", seq %" PRId64 ", seqnow %" PRId64 ". ",
                            rp.length, s, rp.seq, rs->rseq);
                    if (length < 100)
                        fwrite(((char *)data) + offset, 1, rp.length, stderr);
                    fprintf(stderr, "\n");
#endif /*DEBUG_RSOCKET*/
                    rs->rseq = rp.seq + 1;
                    break;
        ...
        fprintf(stderr,
                "[Network] Packet receive retry count at: %" PRId64 "\n",
                retry);
        rs = repair_connection(&s);
    }
    return length;
