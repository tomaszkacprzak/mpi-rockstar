...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        // See io/bgc2.h for a description of the group and particle structure
        // format
    }
    return 0;
}
...
void load_full_particles(char *filename, struct halo **h, int64_t *num_h,
    ...
    SHORT_PARSETYPE;
    enum short_parsetype stypes_h[20] = {D64, D64, D64, F, F, F, F, F, F, F,
                                         F,   F,   F,   F, F, F, F, F, F, F};
    enum parsetype       types_h[20];
    enum short_parsetype stypes_p[10] = {F, F, F, F, F, F, D64, D64, D64, D64};
    enum parsetype       types_p[10];

    void *data_h[20] = {
        &(the_h.id),    &(the_h.flags), &(the_h.num_p),  &(the_h.m),
    ...
    void *data_p[10] = {the_p.pos,     the_p.pos + 1,  the_p.pos + 2,
                        ...
                        &(the_p.id),   &(the_p.a_hid), &(the_p.hid),
                        &(the_p.ehid)};

    input = check_fopen(filename, "r");
    for (i = 0; i < 20; i++)
        types_h[i] = stypes_h[i];
    for (i = 0; i < 10; i++)
        types_p[i] = stypes_p[i];
    ...
    while (fgets(buffer, 1024, input)) {
        ...
        if (!strncmp(buffer, "#Bounds: ", 9))
            sscanf(buffer, "#Bounds: (%f, %f, %f) - (%f, %f, %f)", bnds,
                   bnds + 1, bnds + 2, bnds + 3, bnds + 4, bnds + 5);
    ...
    while (fgets(buffer, 1024, input)) {
        ...
        if (!((*num_p) % 1000)) {
            *p = check_realloc(*p,
                               sizeof(struct full_particle) * ((*num_p) + 1000),
