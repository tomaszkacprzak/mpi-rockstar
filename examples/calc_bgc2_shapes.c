...
int main(int argc, char **argv) {
    ...
    if (argc < 9) {
        printf("Usage: %s mass_definition radius_fraction weighted_shapes "
               "force_resolution shape_iterations bound_particles use_com "
               "file1.bgc2 ...\n",
               argv[0]);
        printf("E.g.: %s vir 0.5 1 0.001 10 1 1 file1.bgc2 #calculates "
               "weighted shape for bound particles within 0.5 Rvir, assuming a "
               "force resolution of 0.001 Mpc/h, and does up to 10 iterations, "
               "using the halo center-of-mass within the chosen radius.\n",
               argv[0]);
        exit(1);
    }
    ...
    printf("#HaloID HaloParent Mass Radius Vmax X Y Z VX VY VZ "
           "Mass_within_%gxR%s A[x] A[y] A[z] b_to_a c_to_a\n",
           rfrac, argv[1]);
    ...
    printf("#HaloParent: the ID of a larger halo containing this halo, if any; "
           "-1 otherwise\n");
    printf("#Shapes calculated within %g x R%s\n", rfrac, argv[1]);
    printf("#Shapes are %s\n", (WEIGHTED_SHAPES ? "weighted" : "unweighted"));
    if (BOUND_PROPS)
        printf("#Only bound particles are used.\n");
    else
        printf("#Both bound and unbound particles are used.\n");
    ...
    printf("#b_to_a, c_to_a: Ratio of second and third largest shape ellipsoid "
           "axes (B and C) to largest shape ellipsoid axis (A) "
           "(dimensionless).\n#  Shapes are determined by the method in "
           "Allgood et al. (2006).\n#A[x],A[y],A[z]: Largest shape ellipsoid "
           "axis (kpc/h comoving)\n#All particles are considered (bound and "
           "unbound) so these measurements\n#  may differ from the Rockstar "
           "catalogs, especially for subhalos.\n");
    for (i = 8; i < argc; i++) {
        ...
        double dens_thresh =
            particle_thresh_dens[0] * (4.0 * M_PI / 3.0) * hdr.part_mass;
        for (j = 0; j < hdr.ngroups; j++) {
            struct halo h;
            memset(&h, 0, sizeof(struct halo));
            check_realloc_var(po, sizeof(struct potential), max_p,
                              grps[j].npart);
            for (k = 0; k < grps[j].npart; k++) {
                memcpy(po[k].pos, parts[p_start + k].pos, sizeof(float) * 3);
                memcpy(po[k].pos + 3, parts[p_start + k].vel,
                       sizeof(float) * 3);
                for (int64_t l = 0; l < 3; l++)
                    bulkvel[l] += po[k].pos[l + 3];
                po[k].pe = po[k].ke = 0;
                po[k].flags         = 0;
            ...
            if (USE_COM && h.r)
                for (int64_t l = 0; l < 3; l++)
                    h.pos[l] = pos[l] / (double)(k + 1);
            calc_shape(&h, k, BOUND_PROPS);
            printf("%" PRId64 " %" PRId64
                   " %e %f %f %f %f %f %f %f %f %e %f %f %f %f %f\n",
                   grps[j].id, grps[j].parent_id, grps[j].mass,
                   1e3 * grps[j].radius, grps[j].vmax, grps[j].pos[0],
                   grps[j].pos[1], grps[j].pos[2], grps[j].vel[0],
                   grps[j].vel[1], grps[j].vel[2], (k + 1) * PARTICLE_MASS,
                   h.A[0], h.A[1], h.A[2], h.b_to_a, h.c_to_a);
